...
register(Action(
    ...
    description=(
        ...
        "Spotify'). Extract the actual subject — the song / album / "
        ...
        "ALWAYS fill params.query when the user named anything — "
        "don't leave it empty just because the query is ambiguous.\n"
        ...
        "Otherwise leave kind unset and Spotify will pick the best "
        "match across all four types based on the query.\n"
        "Optional: for genuinely AMBIGUOUS queries (the same name "
        "could mean multiple unrelated things, e.g. a common track "
        "title), you may call search_spotify first to see top "
        "matches and then ALSO set params.uri to the chosen specific "
        "result — keep params.query populated for narration. Skip "
        "search_spotify for clear queries; the internal search "
        "handles those."
    ),
    triggers={
        "en": [
