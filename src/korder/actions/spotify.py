...
register(Action(
    name="spotify_play",
    description=(
        "Play music on Spotify by query. The word 'Spotify' may "
        "appear anywhere in the utterance — start, end, or middle. "
        "Extract the song / album / artist / playlist name into "
        "params.query (strip out 'Spotify', the play verb, and any "
        "kind cue word; what's left is the query, even if it's an "
        "English title in a Polish frame). ALWAYS fill params.query "
        "when the user named anything. "
        "USE for play requests. SKIP for 'tell me about X' / 'co "
        "wiesz o X' (those are conversational, answer in response). "
        "OPTIONAL: for truly ambiguous queries, call search_spotify "
        "first and dispatch with params.uri set to a chosen result."
    ),
    triggers={
        "en": [
            "play on spotify",
