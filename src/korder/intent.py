...
_SYSTEM_PROMPT = (
    ...
    "  - Action has another missing parameter (`query`, `kind`, etc.) "
    "AND you could NOT extract it from the input: ask WHAT the user "
    "wants — e.g. 'What do you want to play?'. CRITICAL: if you DID "
    "extract a non-empty value into params (e.g. `params: {query: "
    "'Lose Yourself'}`), the action will execute — leave `response` "
    "empty. Don't emit a 'what do you want' question alongside an "
    "action whose params you already filled.\n"
